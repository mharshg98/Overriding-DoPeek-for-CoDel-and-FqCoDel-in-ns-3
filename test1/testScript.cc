...
CoDelPeekTest::DoRun(){
  ...
  Enqueue(queue, pktSize, 5);
  NS_LOG_UNCOND("Enqueue Done");

  Time firstDequeueTime = 2 * queue->GetTarget();
  Simulator::Schedule(firstDequeueTime, &CoDelPeekTest::Dequeue, this, queue);
...
CoDelPeekTestMark::DoRun(){
  ...
  Time firstDequeueTime = 2 * queue->GetTarget();
  Simulator::Schedule(firstDequeueTime, &CoDelPeekTestMark::Dequeue, this, queue);
